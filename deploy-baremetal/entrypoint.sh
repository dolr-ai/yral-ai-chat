...
# Expand env vars in Litestream config template
LITESTREAM_CONFIG="/tmp/litestream.yml"
envsubst < /etc/litestream.yml > "$LITESTREAM_CONFIG"
...
if [ "${ENABLE_LITESTREAM}" = "true" ] && [ ! -f /app/data/yral_chat.db ]; then
    ...
    litestream restore -if-replica-exists -config "$LITESTREAM_CONFIG" /app/data/yral_chat.db || {
...
if [ "${ENABLE_LITESTREAM}" = "true" ]; then
    ...
    exec litestream replicate -config "$LITESTREAM_CONFIG" -exec "/app/yral-ai-chat"
else
