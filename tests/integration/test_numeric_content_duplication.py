...
def test_numeric_content_duplication_bug(client, auth_headers):
    ...
    create_response = client.post(
        "/api/v1/chat/conversations", json={"influencer_id": AHAAN_INFLUENCER_ID}, headers=auth_headers
    )
    ...
    try:
        ...
        assert user_msg["content"] == "70", f"User message content should be '70' but got '{user_msg['content']}'"
...
def test_numeric_content_duplication_with_string(client, auth_headers):
    ...
    try:
        ...
        assert "7070" not in assistant_content, f"❌ BUG: Response contains '7070': {assistant_content}"
        assert not re.search(r"70\s+70", assistant_content), f"❌ BUG: Response contains '70 70': {assistant_content}"
