...
def test_message_content_not_duplicated(client, clean_conversation_id, auth_headers):
    ...
    assert "8080" not in user_msg["content"], f"Content was incorrectly duplicated to '{user_msg['content']}'"
    assert (
        "8080" not in found_message["content"]
...
def test_short_numeric_message_preserved(client, clean_conversation_id, auth_headers):
    ...
    for test_content in test_cases:
        ...
        user_msg = data["user_message"]
        assert user_msg["content"] == test_content, f"Content '{test_content}' was modified to '{user_msg['content']}'"
        ...
        assert (
            stored_user_msg["content"] == test_content
