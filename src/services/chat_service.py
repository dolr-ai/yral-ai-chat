...
class ChatService:
    ...
    async def _hydrate_existing_conversation(
        self, conversation: Conversation, influencer: AIInfluencer
    ) -> Conversation:
        ...
        conversation.last_message = await self.conversation_repo._get_last_message(conversation.id)
        conversation.message_count = await self.message_repo.count_by_conversation(conversation.id)
        return conversation
    ...
    async def assemble_ai_context(
        self, conversation_id: str, user_message_id: str, influencer: AIInfluencer, memories: dict[str, object]
    ) -> tuple[list[Message], str]:
        """Fetch history and build system instructions"""
        all_recent = await self.message_repo.get_recent_for_context(conversation_id=conversation_id, limit=11)
    ...
    async def send_message(self, params: SendMessageParams) -> tuple[Message, Message, bool]:
        ...
        memories_val = conversation.metadata.get("memories", {})
        memories = memories_val if isinstance(memories_val, dict) else {}

        # 2. Deduplication Check
        if params.client_message_id:
            existing_user_msg = await self.message_repo.get_by_client_id(
                params.conversation_id, params.client_message_id
            )
        ...
        await self._trigger_post_message_tasks(
            params, conversation, ai_input_content, response_text, memories, influencer, assistant_message
        )
    ...
    async def _broadcast_typing(self, user_id: str, conversation_id: str, influencer_id: str, is_typing: bool):
        ...
        try:
            await manager.emit_typing_status_event(
                user_id=user_id, conversation_id=conversation_id, influencer_id=str(influencer_id), is_typing=is_typing
            )
        except Exception as e:
    ...
    async def _handle_message_notifications(
        ...
        try:
            ...
            updated_conversation = await self.conversation_repo.get_by_id(str(conversation_id))
            ...
            await manager.emit_new_message_event(
                ...
                message=MessageResponse(
                    ...
                    audio_duration_seconds=assistant_message.audio_duration_seconds,
    ...
    async def generate_image_for_conversation(
        self, conversation_id: str, user_id: str, prompt: str | None = None
    ) -> Message:
        """
        ...
        if not final_prompt:
            ...
            messages = await self.message_repo.list_by_conversation(
                conversation_id=conversation.id, limit=10, order="desc"
            )
        ...
        image_url = await self.replicate_client.generate_image_via_image(
            prompt=final_prompt, aspect_ratio="9:16", input_image=influencer.avatar_url or ""
        )
        ...
        s3_key, _, _ = await self.storage_service.save_file(
            file_content=image_data, filename=f"generated_{UUID(int=0)}.jpg", user_id=user_id
        )
    ...
    async def list_conversations(
        self, user_id: str, influencer_id: str | None = None, limit: int = 20, offset: int = 0
        ...
        if conversations:
            ...
            recent_messages_map = await self.message_repo.get_recent_for_conversations_batch(
                conversation_ids=conversation_ids, limit_per_conv=10
            )
    ...
    async def list_messages(
        self, conversation_id: str, user_id: str, limit: int = 50, offset: int = 0, order: str = "desc"
    ...
    async def mark_conversation_as_read(self, conversation_id: str, user_id: str) -> dict[str, object]:
        ...
        result = {"id": str(conversation_id), "unread_count": 0, "last_read_at": read_at}

        # Broadcast conversation read event via WebSocket
        try:
            await manager.emit_conversation_read_event(
                user_id=user_id, conversation_id=str(conversation_id), read_at=read_at.isoformat()
            )
        except Exception as e:
    ...
    async def _save_message(self, **kwargs) -> Message:
        ...
        try:
            ...
            if "unique" in msg and "client_message_id" in msg:
                logger.warning(
                    f"Deduplication triggered at DB level for client_message_id: {kwargs.get('client_message_id')}"
                )
