...
class ChatService:
    ...
    async def create_conversation(self, user_id: str, influencer_id: UUID) -> tuple[Conversation, bool]:
        ...
        if influencer.initial_greeting:
            ...
            try:
                ...
                logger.error(
                    f"Failed to create initial greeting message for conversation {conversation.id}: {e}", exc_info=True
                )
        else:
    ...
    async def send_message(self, params: SendMessageParams) -> tuple[Message, Message]:
        ...
        if params.message_type in [MessageType.IMAGE, MessageType.MULTIMODAL] and params.media_urls:
            media_urls_for_ai = self._convert_media_urls_for_ai(params.media_urls)
        ...
        try:
            ...
            if "foreign key" in str(e).lower():
                logger.warning(
                    f"Failed to save assistant message: Conversation {params.conversation_id} was deleted. " f"User: {params.user_id}"
                )
